...
public class ErrorTest {
    ...
    public void testError() {
        Server server = new Server(org.glassfish.websocket.test.basic.bean.ErrorTestBean.class);
        server.start();
        try {
            messageLatch = new CountDownLatch(1);

            WebSocketClient client = WebSocketClient.createClient();
            client.openSocket("ws://localhost:8025/websockets/tests/error", 10000, new EndpointAdapter() {
                @Override
