...
public class ERBoolean {
    ...
    @WebSocketMessage
    public void handleIncomingBoolean(boolean b, BooleanRemote booleanRemote) {
        this.lastBoolean = b;
        makeCallback(booleanRemote);
    }

    private void makeCallback(BooleanRemote remote) {
        try {
            Thread.sleep(100);
            remote.sendBooleanMessage(lastBoolean);
        } catch (Throwable t) {
            t.printStackTrace();
        }
