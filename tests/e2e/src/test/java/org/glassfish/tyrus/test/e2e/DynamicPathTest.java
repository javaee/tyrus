...
public class DynamicPathTest {
    ...
    public void testDynamicPath() {
        Server server = new Server(org.glassfish.tyrus.test.e2e.bean.DynamicPathTestBean.class);
        server.start();
    ...
    private void testPath(String segmentPath, final String message, final String response) {
        ...
        try {
            final DefaultClientEndpointConfiguration.Builder builder = new DefaultClientEndpointConfiguration.Builder("ws://localhost:8025/websockets/tests/dynamicpath" + segmentPath);
